...
export function filterAndSortServers(servers, query = "", category = "all", sort = "recent", activeOnly = false) {
  ...
  let result = [...servers].filter((server) => {
    ...
    const matchesActivity = !activeOnly || Number(server.communityOnline || 0) > 0;

    return matchesCategory && matchesQuery && matchesActivity;
  });
  ...
  if (sort === "top") {
    ...
    result.sort((a, b) => Number(b.communityOnline || 0) - Number(a.communityOnline || 0));
  } else if (sort === "members") {
    result.sort((a, b) => Number(b.communityTotal || 0) - Number(a.communityTotal || 0));
  } else if (sort === "name") {
