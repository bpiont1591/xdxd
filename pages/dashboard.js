...
export default function Dashboard() {
  ...
  async function saveServer(e) {
    ...
    try {
      ...
      setNotice(data.savedInFallbackJson
        ? "Zapisano zmiany. Prisma padła, więc zapis poszedł awaryjnie do data/servers.json."
        : "Zapisano zmiany.");
    } catch (err) {
  ...
  if (status === "loading") {
    return <main className="panel-page"><div className="panel-only glass">Ładowanie sesji...</div></main>;
  }
    ...
    return (
      <main className="panel-page">
        <div className="panel-only glass">
          ...
          <p className="muted">Logowanie pobiera serwery użytkownika. Dodanie bota to osobny krok.</p>
          <div className="button-row">
            <button className="btn btn-primary" onClick={() => signIn("discord", { callbackUrl: "/dashboard" })}>
  ...
  return (
    <>
      <Head><title>Dashboard • DiscordBoard Premium</title></Head>
        ...
        <aside className="sidebar glass dashboard-sidebar slim-sidebar visible-sidebar">
          <div className="sidebar-header deluxe">
            <Link href="/" className="brand brand-link">
              ...
              <div><strong>DiscordBoard</strong><span>Strona główna</span></div>
            </Link>
            <button className="btn btn-ghost" onClick={() => signOut({ callbackUrl: "/" })}>Wyloguj</button>
          </div>
          ...
          <div className="profile-card glass-lite compact-profile">
            <div className="profile-card-top">
              <div className="profile-avatar-wrap">
                {session?.user?.image ? (
                  <img className="profile-avatar-img" src={session.user.image} alt={session?.user?.name || "Avatar"} />
                ) : (
                  <div className="user-avatar deluxe-avatar">{session?.user?.name?.slice(0, 1) || "U"}</div>
                )}
              </div>
          ...
          <div className="sidebar-block">
            ...
            <div className="button-row" style={{ marginBottom: 12 }}>
              ...
              >
                {showServerPicker ? "Ukryj listę" : "Dodaj serwer"}
              </button>
            </div>

            {showServerPicker ? (
              loading ? (
                <div className="server-list"><div className="server-item active">Ładowanie serwerów z Discorda...</div></div>
              ) : servers.length === 0 ? (
                <div className="empty-box">Nie znaleziono serwerów, gdzie jesteś ownerem albo masz Manage Server.</div>
              ) : (
                  ...
                  {servers.map((server) => (
                    ...
                    >
                      <div className="server-icon">
                        {server.icon ? (
                          <img src={`https://cdn.discordapp.com/icons/${server.id}/${server.icon}.png?size=128`} alt={server.name} />
                        ) : (
              ...
              <div className="empty-box">Kliknij „Dodaj serwer”, aby wybrać serwer z listy.</div>
            )}
          </div>
        </aside>
        ...
        <section className="content-panel">
          {selectedServer ? (
            <>
              <div className="dashboard-banner glass compact-banner">
                <div className="banner-main">
                  <div className="server-avatar xl">
                    {selectedServer.icon ? (
                      <img src={`https://cdn.discordapp.com/icons/${selectedServer.id}/${selectedServer.icon}.png?size=256`} alt={selectedServer.name} />
                    ) : (
                ...
                <div className="button-row compact-actions">
                  {!selectedServer.botInstalled ? (
                    <a className="btn btn-primary" href={botAddUrl} target="_blank" rel="noreferrer">Dodaj bota</a>
                  ) : (
                    <button className="btn btn-primary" type="button" disabled>Bot już dodany</button>
                  )}
                  <button className="btn btn-ghost" type="button" onClick={loadServers}>Odśwież</button>
                </div>
              </div>

              {selectedServer.botStatusError ? (
                <div className="panel-card glass" style={{ marginBottom: 20 }}>
                  <p className="muted"><strong>Problem ze statusem bota:</strong> {selectedServer.botStatusError}</p>
                </div>
              ) : null}
              ...
              <div className="panel-card glass editor-card-deluxe">
                <div className="panel-head simple">
                  <div><span className="badge">edycja</span><h1>Konfiguracja serwera</h1></div>
                </div>
            ...
            <div className="panel-card glass">
              ...
              <p className="muted">Kliknij „Dodaj serwer”, aby wybrać serwer z listy.</p>
              <div className="button-row">
                <button className="btn btn-primary" type="button" onClick={() => setShowServerPicker(true)}>
                  Dodaj serwer
                </button>
